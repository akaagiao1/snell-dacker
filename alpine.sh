...
install_dependencies() {
    ...
    # 下载 glibc 签名密钥
    echo -e "${CYAN}下载 glibc 签名密钥...${RESET}"
    curl -sL -o /etc/apk/keys/sgerrand.rsa.pub https://alpine-pkgs.sgerrand.com/sgerrand.rsa.pub
    
    # 只下载核心的 glibc 包，跳过非必要包
    echo -e "${CYAN}下载 glibc 核心包...${RESET}"
    cd /tmp
    curl -sL -o glibc.apk "https://github.com/sgerrand/alpine-pkg-glibc/releases/download/${GLIBC_VERSION}/glibc-${GLIBC_VERSION}.apk"
    curl -sL -o glibc-bin.apk "https://github.com/sgerrand/alpine-pkg-glibc/releases/download/${GLIBC_VERSION}/glibc-bin-${GLIBC_VERSION}.apk"
    
    # 验证下载的文件
    for file in glibc.apk glibc-bin.apk; do
        if [ ! -f "$file" ]; then
            echo -e "${RED}$file 下载失败！${RESET}"
            return 1
        fi
        if [ ! -s "$file" ]; then
            echo -e "${RED}$file 文件为空，下载可能失败！${RESET}"
            return 1
        fi
    done
    
    echo -e "${CYAN}安装 glibc 包...${RESET}"
    apk add --allow-untrusted glibc.apk glibc-bin.apk
    
    # 跳过耗时的语言环境配置（非必需步骤）
    echo -e "${YELLOW}跳过语言环境配置（非必需步骤）...${RESET}"
    
    # 创建环境变量配置文件
    echo -e "${CYAN}配置环境变量...${RESET}"
    cat > /etc/profile.d/snell-globals.sh << 'EOF'
...
EOF
    ...
    # 立即加载环境变量
    . /etc/profile.d/snell-globals.sh
    ...
    if [ -f "/usr/glibc-compat/lib/ld-linux-x86-64.so.2" ]; then
        echo -e "${GREEN}✓ glibc 安装验证成功${RESET}"
    else
...
if [ "$(id -u)" != "0" ]; then 
    echo -e "${RED}请以 root 权限运行此命令 (e.g., sudo snell)${RESET}"
    exit 1
fi

echo -e "${CYAN}正在从 GitHub 获取最新的管理脚本...${RESET}"
TMP_SCRIPT=$(mktemp)
if curl -sL "https://raw.githubusercontent.com/jinqians/snell.sh/main/snell-alpine.sh" -o "$TMP_SCRIPT"; then
    sh "$TMP_SCRIPT"
    rm -f "$TMP_SCRIPT"
else
...
install_snell() {
    ...
    if ! install_dependencies; then
        echo -e "${RED}依赖安装失败，请检查网络连接和系统状态。${RESET}"
        exit 1
    fi
    ...
    echo -e "${YELLOW}下载 URL: ${SNELL_URL}${RESET}"
    
    mkdir -p "${INSTALL_DIR}"
    cd /tmp
    if ! curl -L -o snell-server.zip "${SNELL_URL}"; then
...
export LD_LIBRARY_PATH="/usr/glibc-compat/lib:${LD_LIBRARY_PATH}"
export GLIBC_TUNABLES="glibc.pthread.rseq=0"
exec /usr/glibc-compat/lib/ld-linux-x86-64.so.2 /usr/local/bin/snell-server "$@"
EOF
...
EOF
    ...
    rc-update add snell default > /dev/null 2>&1
    ...
    if rc-service snell status > /dev/null 2>&1; then
        ...
        echo -e "${YELLOW}尝试手动启动: rc-service snell start${RESET}"
    fi
}
...
uninstall_snell() {
    ...
    rm -f ${INSTALL_DIR}/snell-server ${INSTALL_DIR}/snell-server-wrapper
    rm -rf ${SNELL_CONF_DIR} /var/log/snell
    rm -f /usr/local/bin/snell
    rm -f /etc/profile.d/snell-globals.sh
...
show_information() {
    ...
    IPV4_ADDR=$(curl -s4 --connect-timeout 5 https://api.ipify.org 2>/dev/null || echo "无法获取")
    IPV6_ADDR=$(curl -s6 --connect-timeout 5 https://api64.ipify.org 2>/dev/null || echo "无法获取")
    ...
    if [ -n "$IPV4_ADDR" ] && [ "$IPV4_ADDR" != "无法获取" ]; then
        IP_COUNTRY_IPV4=$(curl -s --connect-timeout 5 "http://ipinfo.io/${IPV4_ADDR}/country" 2>/dev/null || echo "Unknown")
    ...
    if [ -n "$IPV6_ADDR" ] && [ "$IPV6_ADDR" != "无法获取" ]; then
        IP_COUNTRY_IPV6=$(curl -s --connect-timeout 5 "https://ipapi.co/${IPV6_ADDR}/country/" 2>/dev/null || echo "Unknown")
    ...
    echo -e "${YELLOW}Snell 版本: ${RESET}${INSTALLED_VERSION_CHOICE}"
    echo -e "\n${YELLOW}配置文件: ${RESET}${SNELL_CONF_FILE}"
    echo -e "${YELLOW}日志文件: ${RESET}/var/log/snell/snell.log"
    echo -e "${YELLOW}管理命令: ${RESET}sudo snell"
    echo -e "${BLUE}============================================${RESET}"
}
...
check_status() {
    ...
    echo -e "\n${CYAN}=== 进程状态 ===${RESET}"
    pgrep -af snell-server || echo "未找到 Snell 进程"
